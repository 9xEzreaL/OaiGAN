...
import torch
from models.genre.blocks.spade_normalization import SPADE_Shoutcut
...
class SPADEUp(nn.Module):
    def __init__(self, opt, in_size, out_size, dropout=0.0, first=False):
        ...
        if not first:
            self.norm = SPADE_Shoutcut(spade_config_str, out_size, parsing_nc, "res2", False) # spade_mode can be 'org', 'res', 'concat', 'res2'
        else:
            self.norm = SPADE_Shoutcut(spade_config_str, out_size, parsing_nc, "res2") # spade_mode can be 'org', 'res', 'concat', 'res2'
